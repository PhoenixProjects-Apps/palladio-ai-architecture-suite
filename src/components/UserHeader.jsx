...
export default function UserHeader() {
  ...
  return (
  <div className="md:hidden grid grid-col-4 gap-3">
    ...
    <Button variant="outline" size="icon" onClick={() => setIsMobileOpen(true)}>
      ...
      <Link to={createPageUrl('PalladioPricing')} className="w-10 h-10 gap-2 text-sm font-semibold text-amber-300 hover:text-amber-200 bg-amber-500/10 hover:bg-amber-500/20 px-4 py-2 rounded-full transition border border-amber-500/20">
        <Coins size={16} />
        {user ? (user.tokens ?? 0) : '—'}
      </Link>
      {user ? (
